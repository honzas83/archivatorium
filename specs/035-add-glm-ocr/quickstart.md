...
Expected outcome: tests verify the exact GLM request, all five defaults and overrides, model independence, no-context behavior for multipage/resume/retry flows, invalid-input rejection, and unchanged standard behavior.
...
Expected outcome: help lists `--mode [standard|glm]` with default `standard` and the five inference override options while retaining `INPUT_DIR OUTPUT_DIR`, the existing `--model` default, and all prior OCR options. See [contracts/cli.md](contracts/cli.md) for the complete contract.
...
- Focused OCR validation: `40 passed`; GLM request, standard regression, model independence, overrides, resume, retry, and CLI rejection scenarios all passed.
- Full project validation: `281 passed, 1 skipped`; source-package coverage was 89%, including 93% for `archivatorium/ocr_engine.py` and 94% for `archivatorium/cli.py`.
- `ruff check .`: passed.
...
- `archivatorium ocr --help`: verified `--mode [standard|glm]`, default `standard`, both positional directories, independent `--model`, and all five inference options.
